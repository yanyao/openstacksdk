...
class ShareGroupTest(base.BaseSharedFileSystemTest):
    ...
    def tearDown(self):
        ...
        super(ShareGroupTest, self).tearDown()

    def test_get(self):
        sot = self.operator_cloud.shared_file_system.get_share_group(
            self.SHARE_GROUP_ID)
        assert isinstance(sot, _share_group.ShareGroup)
        self.assertEqual(self.SHARE_GROUP_ID, sot.id)

    def test_find(self):
        sot = self.operator_cloud.shared_file_system.find_share_group(
    ...
    def test_list_delete_share_group(self):
        s_grps = self.operator_cloud.shared_file_system.share_groups()
        self.assertGreater(len(list(s_grps)), 0)
        for s_grp in s_grps:
            for attribute in ('id', 'name', 'created_at'):
                self.assertTrue(hasattr(s_grp, attribute))

            sot = self.conn.shared_file_system.delete_share_group(
                s_grp
            )
            self.assertIsNone(sot)

    def test_update(self):
        u_gp = self.operator_cloud.shared_file_system.update_share_group(
            self.SHARE_GROUP_ID, description='updated share group')
        get_u_gp = self.operator_cloud.shared_file_system.get_share_group(
            u_gp.id)
        self.assertEqual('updated share group',
                         get_u_gp.description)
